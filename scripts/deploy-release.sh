...
if [[ "$TRAVIS_PULL_REQUEST" == "false" ]]; then
  ...
  $HOME/build/internetpolice-eu/TeleportMagic/target/mvn-repo/ \
  travis@travis.internetpolice.eu:WWW/repo/

  echo -e "Publishing final plugin release...\n"

  rsync -r --quiet -e "ssh -p 2222 -o StrictHostKeyChecking=no" \
  $HOME/build/internetpolice-eu/TeleportMagic/target/TeleportMagic-*.jar \
  travis@travis.internetpolice.eu:WWW/downloads/TeleportMagic/
